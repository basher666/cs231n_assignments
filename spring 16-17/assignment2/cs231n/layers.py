...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        test_xhat=(x-running_mean)/np.sqrt(running_var+eps)
        out=gamma*test_xhat+beta
...
def batchnorm_backward(dout, cache):
    ...
    dsample_var = np.sum(dx_hat * (cache['X']-cache['batch_mean']) * (-0.5) * (cache['batch_var'] + cache['eps'])**(-1.5), axis=0)
    dsample_mean = np.sum(dx_hat * (-1/np.sqrt(cache['batch_var'] + cache['eps'])) , axis=0) + dsample_var * ((np.sum(-2*(cache['X']-cache['batch_mean']))) / N)

    dx = dx_hat * (1/np.sqrt(cache['batch_var'] + cache['eps'])) + \
                     dsample_var * (2*(cache['X']-cache['batch_mean'])/N) + \
                            dsample_mean/N
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for j in range(C):
            layer_y=-1
            for k in range(0,H-PH+1,stride):
                layer_y+=1
                layer_x=-1
                for l in range(0,W-PW+1 , stride):
                    layer_x+=1
                    x_tmp = x[i,j,k:k+PH,l:l+PW]
                    mm=np.amax(x_tmp)
                    out[i,j,layer_y,layer_x]=mm
        ...
        for j in range(C):
            ...
            for k in range(0,H-PH+1,stride):
                ...
                for l in range(0,W-PW+1 , stride):
                    layer_x+=1
                    x_tmp = x[i,j,k:k+PH,l:l+PW]
                    ii, jj = np.where(x_tmp == np.max(x_tmp))
                    dx[i,j,k+ii,l+jj] = dout[i,j,layer_y,layer_x]
